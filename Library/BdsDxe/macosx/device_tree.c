...
)
{
  ...
  for (k = 0; k < nodeP->nProperties; ++k) {
    DeviceTreeNodeProperty *propP =
      (DeviceTreeNodeProperty *) (VOID *) startingP;
...
)
{
  ...
  iter =
    (RealDTEntryIterator) AllocatePool (sizeof (struct OpaqueDTEntryIterator));
  if (startEntry != NULL) {
...
)
{
  ...
  iter =
    ...
    AllocatePool (sizeof (struct OpaqueDTPropertyIterator));
  iter->entry = entry;
