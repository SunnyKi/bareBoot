   ...
   )
{
  ...
  if (PlatformData == NULL) return EFI_OUT_OF_RESOURCES;
...
EFI_STATUS SetVariablesForOSX()
{
	...
	UINT32      FwFeatures      = 0xc0007417;
	UINT32      FwFeaturesMask  = 0xc0007fff;
	CHAR8       *None	= "none";
    UINT8       SNLen = 20;
    UINT8       LanguageLen = 11;
  
    CHAR8*      Addr = &gSettings.BoardSerialNumber[19];
    while ((*Addr == ' ') || (*Addr == 0)) { Addr--; SNLen--; }
    Addr	= &gSettings.Language[10];
    while ((*Addr == ' ') || (*Addr == 0)) { Addr--; LanguageLen--; }
...
//	UINT32				KextListSize;
	...
	if (!EFI_ERROR (Status)) 
	{
        ...
        //    if (!FrontSideBus)  FrontSideBus = 100 * Mega;
