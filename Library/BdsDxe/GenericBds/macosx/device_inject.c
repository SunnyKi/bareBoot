...
)
{
  ...
  AsciiSPrint (buffer, len, "%08x%08x%04x%04x", SwapBytes32 (pstring->length), pstring->WHAT2,
               SwapBytes16 (pstring->numentries), pstring->WHAT3);
  buffer += 24;

  while (i < pstring->numentries) {
    UINT8 *dataptr;

    AsciiSPrint (buffer, len, "%08x%04x%04x", SwapBytes32 (pstring->entries[i]->length),
                 SwapBytes16 (pstring->entries[i]->numentries), pstring->entries[i]->WHAT2); //FIXME: wrong buffer sizes!
    buffer += 16;
    AsciiSPrint (buffer, len, "%02x%02x%04x%08x%08x", pstring->entries[i]->acpi_dev_path.type,
    ...
    for (x = 0; x < pstring->entries[i]->num_pci_devpaths; x++) {
      AsciiSPrint (buffer, len, "%02x%02x%04x%02x%02x", pstring->entries[i]->pci_dev_path[x].type,
    ...
    AsciiSPrint (buffer, len, "%02x%02x%04x", pstring->entries[i]->path_end.type,
                 pstring->entries[i]->path_end.subtype,
                 SwapBytes16 (pstring->entries[i]->path_end.length));
    buffer += 8;
    dataptr = pstring->entries[i]->data;

    for (x = 0; x < (INT32) ((pstring->entries[i]->length) - (24 + (6 * pstring->entries[i]->num_pci_devpaths))); x++) {
...
)
{
  ...
  UnicodeSPrint (FileName, 24, L"\\EFI\\bareboot\\rom\\%04x_%04x.rom", vendor_id, device_id);
