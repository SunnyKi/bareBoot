 ...
 --*/
{
  ...
  if (!EFI_ERROR (Status)) {
    for (Index = 0; Index < HandleArrayCount; Index++) {
      Status =
        gBS->HandleProtocol (HandleArray[Index], &gEfiPciIoProtocolGuid,
                             (VOID **) &PciIo);
      if (!EFI_ERROR (Status)) {
        //
        // Find the USB host controller
        ...
        if (!EFI_ERROR (Status)) {
          if ((PCI_CLASS_SERIAL == Pci.Hdr.ClassCode[2]) &&
            ...
            case PCI_IF_UHCI:
              ...
              Base = 0;
              Status =
                PciIo->Pci.Read (PciIo, EfiPciIoWidthUint32, 0x20, 1, &Base);
              PortBase = (Base >> 5) & 0x07ff;
              Command = 0x8f00;
              Status =
                PciIo->Pci.Write (PciIo, EfiPciIoWidthUint16, 0xC0, 1,
                                  &Command);
            ...
            case PCI_IF_EHCI:
              ...
              Base = 0;
              Status =
                PciIo->Pci.Read (PciIo, EfiPciIoWidthUint32, 0x10, 1, &Base);
              ...
              //UEFI
              Status = PciIo->Mem.Read (PciIo, EfiPciIoWidthUint32, 0,  //EHC_BAR_INDEX
              ...
              Status =
                PciIo->Pci.Read (PciIo, EfiPciIoWidthUint32, ExtendCap, 1,
                                 &usblegsup);
              ...
              PciIo->Pci.Read (PciIo, EfiPciIoWidthUint32, ExtendCap + 0x4, 1,
                               &usblegctlsts);
              //
              // Disable the SMI in USBLEGCTLSTS firstly
              //
              usblegctlsts &= 0xFFFF0000;
              PciIo->Pci.Write (PciIo, EfiPciIoWidthUint32, ExtendCap + 0x4, 1,
                                &usblegctlsts);
              ...
              *((UINT32 *) (UINTN) (opaddr + 4)) = 0x1000;  //usbsts - clear status registers
              Value = 1;
              PciIo->Pci.Write (PciIo, EfiPciIoWidthUint32, ExtendCap, 1,
                                &Value);
              ...
              if (isOwnershipConflict) {
                Value = 0;
                PciIo->Pci.Write (PciIo, EfiPciIoWidthUint8, ExtendCap + 3, 1,
                                  &Value);
                TimeOut = 40;
                while (TimeOut--) {
                  gBS->Stall (500);
              ...
              if (isOwnershipConflict) {
                ...
                // Hard reset
                Value = 0;
                PciIo->Pci.Write (PciIo, EfiPciIoWidthUint8, ExtendCap + 2, 1,
                                  &Value);
                TimeOut = 40;
                while (TimeOut--) {
                  gBS->Stall (500);
                ...
                PciIo->Pci.Read (PciIo, EfiPciIoWidthUint32, ExtendCap + 0x4, 1,
                                 &usblegctlsts);
                usblegctlsts &= 0xFFFF0000;
                PciIo->Pci.Write (PciIo, EfiPciIoWidthUint32, ExtendCap + 0x4,
                                  1, &usblegctlsts);
              }
              if (Value & 0x00010000) {
