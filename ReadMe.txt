...
2.1 Build Duet Platform module   
  ...
  2). run "edksetup.bat"
  3). run "build -p Duet\Duet.dsc -a IA32" for IA32 architecture platform or 
          "build -p Duet\Duet.dsc -a X64" for X64 architecture platform.
 
2.2 Execute post build actions  
  1). enter <Workspace>\Duet directory.
...
2.1 Build Duet Platform module   
  ...
  3). run "build -p Duet/Duet.dsc -a IA32 -t UNIXGCC" for IA32 architecture platform or 
          "build -p Duet/Duet.dsc -a X64 -t UNIXGCC" for X64 architecture platform.
  
2.2 Execute post build actions  
  1). enter /R9_tree/Duet directory.
  2). run "./PostBuild.sh IA32" for IA32 architecture platform or 
          "./PostBuild.sh X64" for X64 architecture platform.

 NOTE: After post build action, you should check the size of EfiLdr at $WORKSPACE/Build/Duet/DEBUG_UNIXGCC directory, it must less than 470k.
       If not, you should manually remove some unnecessary drivers at Duet.fdf file.
