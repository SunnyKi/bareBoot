...
#ifndef _EFI_OHCI_URB_H_
#define _EFI_OHCI_URB_H_


typedef struct _OHC_QTD  OHC_QTD;
typedef struct _OHC_QH   OHC_QH;
typedef struct _URB      URB;

//
// Transfer types, used in URB to identify the transfer type
//
#define OHC_CTRL_TRANSFER       0x01
#define OHC_BULK_TRANSFER       0x02
#define OHC_INT_TRANSFER_SYNC   0x04
#define OHC_INT_TRANSFER_ASYNC  0x08

#define OHC_QTD_SIG             SIGNATURE_32 ('U', 'S', 'B', 'T')
#define OHC_QH_SIG              SIGNATURE_32 ('U', 'S', 'B', 'H')
#define OHC_URB_SIG             SIGNATURE_32 ('U', 'S', 'B', 'R')

//
// Hardware related bit definitions
//
#define OHC_TYPE_ITD            0x00
#define OHC_TYPE_QH             0x02
#define OHC_TYPE_SITD           0x04
#define OHC_TYPE_FSTN           0x06
...
//
// Fill in the hardware link point: pass in a OHC_QH/QH_HW
// pointer to QH_LINK; A OHC_QTD/QTD_HW pointer to QTD_LINK
//
#define QH_LINK(Addr, Type, Term) \
          ((UINT32) ((OHC_LOW_32BIT (Addr) & 0xFFFFFFE0) | (Type) | ((Term) ? 1 : 0)))

#define QTD_LINK(Addr, Term)      QH_LINK((Addr), 0, (Term))

//
// The defination of OHCI hardware used data structure for
...
typedef struct _USB_ENDPOINT {
  ...
  UINTN                   Type;
  UINTN                   PollRate;   // Polling interval used by OHCI
} USB_ENDPOINT;
...
// transfer is supported. A dummy QH is linked to OHCI AsyncListAddr
// as the reclamation header. New transfer is inserted after this QH.
//
struct _OHC_QH {
  QH_HW                   QhHw;
  UINT32                  Signature;
  OHC_QH                  *NextQh;    // The queue head pointed to by horizontal link
