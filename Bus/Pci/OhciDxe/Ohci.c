...
USB_CLEAR_PORT_MAP mUsbClearPortChangeMap[] = {
  {OHC_PORTSC_CSC, EfiUsbPortConnectChange},
  {OHC_PORTSC_PEC, EfiUsbPortEnableChange},
  {OHC_PORTSC_OCC, EfiUsbPortOverCurrentChange},
  {OHC_PORTSC_PRC, EfiUsbPortResetChange}
};
...
USB_CLEAR_PORT_MAP mUsbHubClearPortChangeMap[] = {
  {OHC_HUB_PORTSC_CSC, EfiUsbPortConnectChange},
  {OHC_HUB_PORTSC_PEC, EfiUsbPortEnableChange},
  {OHC_HUB_PORTSC_OCC, EfiUsbPortOverCurrentChange},
  {OHC_HUB_PORTSC_PRC, EfiUsbPortResetChange},
  {OHC_HUB_PORTSC_BHRC, Usb3PortBHPortResetChange}
};
  ...
  )
{
  ...
  for (Index = 0; Index < MapSize; Index++) {
    if (OHC_BIT_IS_SET (State, mUsbClearPortChangeMap[Index].HwState)) {
      OhcClearRootHubPortFeature (This, PortNumber, mUsbClearPortChangeMap[Index].Selector);
    }
  }
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if ((Request->Request     == USB_REQ_SET_ADDRESS) &&
    ...
    if (Ohc->UsbDevContext[SlotId].OhciDevAddr == 0) {
      Status = EFI_DEVICE_ERROR;
      goto ON_EXIT;
    }
    //
  ...
  *DataLength     = Urb->Completed;
  ...
  if ((Request->Request     == USB_REQ_GET_DESCRIPTOR) &&
    ...
    for (Index = 0; Index < MapSize; Index++) {
      if (OHC_BIT_IS_SET (State, mUsbHubClearPortChangeMap[Index].HwState)) {
