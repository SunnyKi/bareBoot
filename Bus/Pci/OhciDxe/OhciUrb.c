  ...
  )
{
  ...
  Qtd = UsbHcAllocateMem (Ohc->MemPool, sizeof (OHC_QTD));

  if (Qtd == NULL) {
    return NULL;
  }

  Qtd->Signature    = OHC_QTD_SIG;
  Qtd->Data         = Data;
  Qtd->DataLen      = 0;
  ...
  if (Data != NULL) {
    ...
    for (Index = 0; Index <= QTD_MAX_BUFFER; Index++) {
      //
      // Set the buffer point (Check page 41 OHCI Spec 1.0). No need to
      // compute the offset and clear Reserved fields. This is already
      // done in the data point.
      //
      QtdHw->Page[Index]      = OHC_LOW_32BIT (DataPhy);
      QtdHw->PageHigh[Index]  = OHC_HIGH_32BIT (DataPhy);

      ThisBufLen              = QTD_BUF_LEN - (OHC_LOW_32BIT (DataPhy) & QTD_BUF_MASK);
...
/**
  Allocate and initialize a OHCI queue head.

  @param  Ohci                  The OHCI device.
  ...
  )
{
  ...
  Qh = UsbHcAllocateMem (Ohci->MemPool, sizeof (OHC_QH));

  if (Qh == NULL) {
    return NULL;
  }

  Qh->Signature       = OHC_QH_SIG;
  Qh->NextQh          = NULL;
  ...
  case OHC_INT_TRANSFER_ASYNC:
  case OHC_INT_TRANSFER_SYNC:
    //
    // Special initialization for the interrupt transfer
    // to set the S-Mask and C-Mask
    //
    QhHw->NakReload = 0;
    OhcInitIntQh (Ep, QhHw);
    break;

  case OHC_BULK_TRANSFER:
...
/**
  ...
  be used by OHCI interface data structure. Only need
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, Qtds) {
    ...
    UsbHcFreeMem (Ohc->MemPool, Qtd, sizeof (OHC_QTD));
  }
}
  ...
  )
{
  ...
  if (Urb->Qh != NULL) {
    ...
    //
    OhcFreeQtds (Ohc, &Urb->Qh->Qtds);
    UsbHcFreeMem (Ohc->MemPool, Urb->Qh, sizeof (OHC_QH));
  }
  ...
  )
{
  ...
  //
  // OHCI follows the alternet next QTD pointer if it meets
  // a short read and the AlterNext pointer is valid. UEFI
  // OHCI driver should terminate the transfer except the
  // control transfer.
  //
  Toggle    = 0;
  Qh        = Urb->Qh;
  ...
  PhyAddr   = UsbHcGetPciAddressForHostMem (Ohc->MemPool, Ohc->ShortReadStop, sizeof (OHC_QTD));
  ...
  //
  if (Urb->Ep.Type == OHC_CTRL_TRANSFER) {
    Len = sizeof (EFI_USB_DEVICE_REQUEST);
    Qtd = OhcCreateQtd (Ohc, (UINT8 *)Urb->Request, (UINT8 *)Urb->RequestPhy, Len, QTD_PID_SETUP, 0, Ep->MaxPacket);
    ...
    // OHCI meets a short control read, it can resume at the status stage.
    ...
    StatusQtd = OhcCreateQtd (Ohc, NULL, NULL, 0, Pid, 1, Ep->MaxPacket);
    ...
    if (Ep->Direction == EfiUsbDataIn) {
      PhyAddr   = UsbHcGetPciAddressForHostMem (Ohc->MemPool, StatusQtd, sizeof (OHC_QTD));
      AlterNext = QTD_LINK (PhyAddr, FALSE);
    }
  ...
  EFI_LIST_FOR_EACH (Entry, &Qh->Qtds) {
    ...
    NextQtd             = EFI_LIST_CONTAINER (Entry->ForwardLink, OHC_QTD, QtdList);
    PhyAddr             = UsbHcGetPciAddressForHostMem (Ohc->MemPool, NextQtd, sizeof (OHC_QTD));
  ...
  //
  NextQtd           = EFI_LIST_CONTAINER (Qh->Qtds.ForwardLink, OHC_QTD, QtdList);
  PhyAddr           = UsbHcGetPciAddressForHostMem (Ohc->MemPool, NextQtd, sizeof (OHC_QTD));
  Qh->QhHw.NextQtd  = QTD_LINK (PhyAddr, FALSE);
  return EFI_SUCCESS;

ON_ERROR:
  OhcFreeQtds (Ohc, &Qh->Qtds);
  return EFI_OUT_OF_RESOURCES;
}
  ...
  )
{
  ...
  Ep->Toggle      = Toggle;
  Ep->Type        = Type;
  Ep->PollRate    = OhcConvertPollRate (Interval);
