  ...
  )
{
  ...
  Finished     = FALSE;
  Status       = EFI_SUCCESS;
  Delay        = TimeOut * OHC_1_MILLISECOND;
  InfiniteLoop = FALSE;
  ...
  for (Index = 0; InfiniteLoop || (Index < Delay); Index++) {
    Finished = OhciCheckTdStatus (Ohc, Td, IsLow, QhResult);
  ...
  )
{
  ...
  UsbHcFreeMem (Ohc->MemPool, AsyncReq->QhSw, sizeof (OHCI_QH_SW));
  ...
  )
{
  ...
  //
  OhciCheckTdStatus (Ohc, AsyncReq->FirstTd, AsyncReq->IsLow, &QhResult);
  ...
  )
{
  ...
  do {
    ...
    Finished = OhciCheckTdStatus (Ohc, AsyncReq->FirstTd, AsyncReq->IsLow, &QhResult);
