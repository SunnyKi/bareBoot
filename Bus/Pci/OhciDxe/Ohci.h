/** @file
  ...
  Provides some data structure definitions used by the OHCI host controller driver.
...
typedef struct _USB_OHCI_INSTANCE    USB_OHCI_INSTANCE;
...
//
#define OHC_1_MILLISECOND            (1000)
//
// OHC generic timeout experience values.
// The unit is microsecond, setting it as 10ms.
//
#define OHC_GENERIC_TIMEOUT          (10 * 1000)
//
// OHC reset timeout experience values.
// The unit is microsecond, setting it as 1s.
//
#define OHC_RESET_TIMEOUT            (1000 * 1000)
//
// OHC delay experience value for polling operation.
// The unit is microsecond, set it as 1ms.
//
#define OHC_POLL_DELAY               (1000)
//
// OHC async transfer timer interval, set by experience.
// The unit is 100us, takes 50ms as interval.
//
#define OHC_ASYNC_TIMER_INTERVAL     EFI_TIMER_PERIOD_MILLISECONDS(50)
...
#define OHC_HIGH_32BIT(Addr64)         ((UINT32)(RShiftU64((UINT64)(UINTN)(Addr64), 32) & 0xFFFFFFFF))
...
#define OHCI_INSTANCE_SIG              SIGNATURE_32 ('x', 'h', 'c', 'i')
#define OHC_FROM_THIS(a)               CR(a, USB_OHCI_INSTANCE, Usb2Hc, OHCI_INSTANCE_SIG)
...
struct _USB_DEV_CONTEXT {
  ...
  BOOLEAN                   Enabled;
  //
  // The slot id assigned to the new device through OHCI's Enable_Slot cmd.
  //
  UINT8                     SlotId;
  //
  ...
  //
  // The actual device address assigned by OHCI through Address_Device command.
  //
  UINT8                     OhciDevAddr;
  //
  // The requested device address from UsbBus driver through Set_Address standard usb request.
  // As OHCI spec replaces this request with Address_Device command, we have to record the
  ...
  // through EFI_USB2_HC_PROTOCOL. Ohci driver would be responsible for translating it to actual
  ...
  //
  // The device descriptor which is stored to support OHCI's Evaluate_Context cmd.
  //
  ...
  // These information is used to support OHCI's Config_Endpoint cmd.
  //
  EFI_USB_CONFIG_DESCRIPTOR **ConfDesc;
};

struct _USB_OHCI_INSTANCE {
  UINT32                    Signature;
  ...
  // stop the OHC DMA operation after exit boot service.
  //
  ...
  OHC_HCSPARAMS1            HcSParams1;   ///< Structural Parameters 1
  OHC_HCSPARAMS2            HcSParams2;   ///< Structural Parameters 2
  OHC_HCCPARAMS             HcCParams;    ///< Capability Parameters
  ...
  //
  // Store device contexts managed by OHCI instance
